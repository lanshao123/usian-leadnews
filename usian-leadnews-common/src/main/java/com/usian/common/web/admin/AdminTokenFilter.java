package com.usian.common.web.admin;
    ...
    public ResponseResult checkToken(HttpServletRequest request, HttpServletResponse response){
        ...
        if(StringUtils.isNotEmpty(token)) {
            ...
            if (result == 0||result==-1) {
                ...
                user = findUser(user); // 可以去数据库查询对用的user信息
                logger.info("find userid:[{}] from uri:{}",user.getId(),request.getRequestURI());
                if(user.getId()!=null) {
                    // 重新设置TOKEN
                    if(result==-1) { //快过期
                        String newToken = AppJwtUtil.getToken(Long.parseLong(user.getId() + ""));
                        logger.info("token快过期签发新的token:{}",newToken);
